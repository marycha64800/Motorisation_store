...
{  
  /*
  fonction sert a faire varier la vitesse.
  elle prend e enabel et la direction en parametre et cree une caaleration ou une decelation en fonction de l'etat precedent
  */
  int i;
  if(dir)
  {
...
void engine_move_solo(Engine *eng)
{
  ...
  engine_speed_variation(eng->ctrl_velocity,1);                                                                                                                  //demarage des moteurs avec avec une acceleration defini en configuation dans gestion.h 
  ...
  while(engine_slow_ok != 1)                                                                                        // tant que tout les moteurs ne sont pas ralenti et les stores ne sont pas proche des capteurs
  {
    ...
    if(interval >= eng->travel_time -time_in_one_loop )                                          // attention speedup peu etre modifier donc il dois aussi etre modifier dans l'intervalle 
     {
       engine_speed_variation(eng->ctrl_velocity,0);
       engine_slow_ok = 1;
     }
  }
  while(awning_stop != 1)
  {
    if(digitalRead(eng->sensor_forward)!= 0 || digitalRead(eng->sensor_backward)!= 0)
    {
      analogWrite(eng->ctrl_velocity, LOW);
      awning_stop = 1; 
    }    
  } 
}
